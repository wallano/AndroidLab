...
public class MyCustomView extends View {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		final TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.MyCustomView, defStyle, 0);

		mExampleString = a.getString(R.styleable.MyCustomView_exampleString);
		mExampleColor = a.getColor(R.styleable.MyCustomView_exampleColor,
				mExampleColor);
		// Use getDimensionPixelSize or getDimensionPixelOffset when dealing
		// with
		// values that should fall on pixel boundaries.
		mExampleDimension = a.getDimension(
				R.styleable.MyCustomView_exampleDimension, mExampleDimension);

		if (a.hasValue(R.styleable.MyCustomView_exampleDrawable)) {
			mExampleDrawable = a
					.getDrawable(R.styleable.MyCustomView_exampleDrawable);
